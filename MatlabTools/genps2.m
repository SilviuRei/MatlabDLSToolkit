function [f,amp]=genps2(nume, numfisfr, diam, theta, lambda, indref, tcelsius, optsav, fgr)
%
...
%
% Fata de versiunea 1 foloseste functia Lorentz corectata, florentz2
%
...
if optsav >= 1  % vrea salvat fisierul cu datele
    rez=[f,amp];
    numefis1=[nume,'-',int2str(diam),'-',int2str(nf),'-mdegr-norm-genps2.fps'];
    save(numefis1,'rez','-ascii');  %salveaza fisierul ascii
end
%
if optsav >= 2  % vrea salvat graficul
   numefis2=[nume,'-',int2str(diam),'-',int2str(nf),'-mdegr-norm-genps2.',fgr];
   saveas(1,numefis2);
end