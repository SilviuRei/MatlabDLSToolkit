function [y]=filtru3(nume, ext_fis, fr, deltafr, frmaxfil, opt, fgr);
%
...
%   
%   adauga un f-fr-deltaf-frmaxfil inainte de punct
%   Daca opt>=1 scrie si imaginea pe disc, de tip fgr
% fata de versiunea 2 taie si inregistratrile cu frecventa mai mare decat
% frmax
%
%   Se apleleaza:
%
...
while (narm*fr-hdeltafr) <= frmax
    ...
    for i=1:n
        if ((x(i,1)<=frmaxfil) & ((x(i,1)<frcrt-hdeltafr)|(x(i,1)>frcrt+hdeltafr)))
            j=j+1;
            y(j,:)=x(i,:);
        end
    end %for i
...
numeout=[nume,'f-',int2str(fr),'-',int2str(deltafr),'-',int2str(frmaxfil),'.',ext_fis];
y1=y(1:j,:);
...
ylabel('PS');
if opt >=1
    saveas(1, [nume,'f-',num2str(fr),'-',num2str(deltafr),'-',int2str(frmaxfil),'.',fgr],fgr);
end
%