function [a,d]=run_load_g351(nume,dmin,pas,dmax,nvaldiam,theta,lambda,indref,tcelsius,fs,nf,nt,ngrup,opt);
%
...
for dd=dmin:pas:dmax
    ...
    for jd=1:nvaldiam
        ...
        [t,x,a0,a1]=gents35(nume, d(i), theta, lambda, indref,tcelsius, fs, nf, nt, 0); %nu scrie fisierul
...
%
save([nume,'-',int2str(dmin),'-',int2str(pas),'-',int2str(dmax),'-',int2str(nvaldiam),'_inputg35.fps'],'a','-ascii')
save([nume,'-',int2str(dmin),'-',int2str(pas),'-',int2str(dmax),'-',int2str(nvaldiam),'_d_inputg35.txt'],'d','-ascii')
%
