...
%               pozitiile finale ale particulelorin fisier cu estensia .poz
...
%             daca este 1 scrie la care cadru a ajuns
%   
...
% fiecare tip de SC
%
% versiunea fs ia in final vstokes 0
%
nume_intrare=[nume,'.inn'];
disp('___________________________________');
disp(' ');
disp('                 CHODIN6-fs');
disp('                 Ultima modificare: 21 Mai 2007');
disp(' ');
disp(['Rulare ',nume,',  ',datestr(clock,0),', control ',int2str(control)]);
disp(' ');
disp('___________________________________');
disp(' ');
%
...
vt=vtermic(vcentru, ro, tabs);
mpart=vcentru*ro;   %masa particulei
if vsed==0
    vstks=vstokes(raza, ro, rozero, eta); 
else    %vreau ca valoarea sa fie fixata prin parametrii de intrare, pt simulare rbc
    vstks=vsed; %atentie, este semnul proiectiei pe axa x, pozitiva daca urca
end
vstks=0; %pentru versiunea fs
vstks
%
...
%
    pozsc=movesc3(pozsc,vstks,mpart,tabs,deltat,deltatbrown);
